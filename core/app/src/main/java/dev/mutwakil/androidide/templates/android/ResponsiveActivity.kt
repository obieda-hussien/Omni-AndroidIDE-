...
import dev.mutwakil.androidide.project.manager.PackageHelper
import dev.mutwakil.androidide.project.manager.ProjectManager
import dev.mutwakil.androidide.project.manager.SdkVersionHelper
import dev.mutwakil.androidide.project.manager.builder.*
import dev.mutwakil.androidide.project.manager.builder.module.*
import dev.mutwakil.androidide.project.manager.builder.toplevel.*
import dev.mutwakil.androidide.templates.*
import dev.mutwakil.androidide.templates.AtcInterface
import dev.mutwakil.androidide.templates.android.navigation.responsive.Fragment
import dev.mutwakil.androidide.templates.android.navigation.responsive.ResponsiveSources
import dev.mutwakil.androidide.templates.android.navigation.responsive.ViewModel
...
class ResponsiveActivity : Template {
  ...
  // Asset paths
  private val ASSETS_BASE_PATH = ResponsiveActivity::class.simpleName
  private val ASSETS_RESOURCES_PATH = "$ASSETS_BASE_PATH/resources"
  private val ASSETS_GRADLE_PATH = "$ASSETS_BASE_PATH/gradle"

  private val TAG = ResponsiveActivity::class.simpleName
      ...
      withContext(Dispatchers.IO) {
        try {
          ...
          val topLevelPlugins = buildList {
            add(
                dev.mutwakil.androidide.project.manager.builder.toplevel.GradlePlugin(
          ...
          topLevelGradleWriter.writeToFile(
              projectRoot,
              if (options.useKts)
                  dev.mutwakil.androidide.project.manager.builder.toplevel.GradleFileType.KTS
              else dev.mutwakil.androidide.project.manager.builder.toplevel.GradleFileType.GROOVY,
              topLevelPlugins,
          )
          ...
          val moduleConfig = moduleGradleConfig {
            addPlugin(
                dev.mutwakil.androidide.project.manager.builder.module.GradlePlugin(
                    "alias",
          ...
          moduleGradleWriter.writeToFile(
              appDir,
              if (options.useKts)
                  dev.mutwakil.androidide.project.manager.builder.module.GradleFileType.KTS
              else dev.mutwakil.androidide.project.manager.builder.module.GradleFileType.GROOVY,
              moduleConfig,
          )
          ...
          val mainActivityContent =
              if (options.languageType == LanguageType.KOTLIN)
                  ResponsiveSources.mainActivityKotlin(packageHelper.getPackageId())
              else ResponsiveSources.mainActivityJava(packageHelper.getPackageId())
          ...
          val settingsDir = File(uiDir, "settings")
          val transformDir = File(uiDir, "transform")
          val slideshowDir = File(uiDir, "slideshow")
          val reflowDir = File(uiDir, "reflow")

          createIfNotExists(settingsDir)
          createIfNotExists(transformDir)
          createIfNotExists(slideshowDir)
          createIfNotExists(reflowDir)

          val reflowFragName = "ReflowFragment"
          val settingsFragName = "SettingsFragment"
          val transformFragName = "TransformFragment"
          val slideshowFragName = "SlideshowFragment"

          val reflowModelName = "ReflowViewModel"
          val settingsModelName = "SettingsViewModel"
          val transformModelName = "TransformViewModel"
          val slideshowModelName = "SlideshowViewModel"
          ...
          if (options.languageType == LanguageType.KOTLIN) {
            activityWriter.createFile(
                ...
                reflowFragName,
                "kt",
                Fragment.templateTypeKt(packageHelper.getPackageId(), "reflow"),
            )
            activityWriter.createFile(
                settingsDir,
                settingsFragName,
                "kt",
                Fragment.templateTypeKt(packageHelper.getPackageId(), "settings"),
            )
            activityWriter.createFile(
                transformDir,
                transformFragName,
                "kt",
                Fragment.templateTypeTransformKt(packageHelper.getPackageId()),
            )
            activityWriter.createFile(
                slideshowDir,
                slideshowFragName,
                "kt",
                Fragment.templateTypeKt(packageHelper.getPackageId(), "slideshow"),
            )
            ...
            activityWriter.createFile(
                ...
                reflowModelName,
                "kt",
                ViewModel.templateTypeKt(packageHelper.getPackageId(), "reflow"),
            )
            activityWriter.createFile(
                slideshowDir,
                slideshowModelName,
                "kt",
                ViewModel.templateTypeKt(packageHelper.getPackageId(), "slideshow"),
            )
            activityWriter.createFile(
                transformDir,
                transformModelName,
                "kt",
                ViewModel.templateTypeTransformKt(packageHelper.getPackageId()),
            )
            activityWriter.createFile(
                settingsDir,
                settingsModelName,
                "kt",
                ViewModel.templateTypeKt(packageHelper.getPackageId(), "settings"),
            )
          } else {
            activityWriter.createFile(
                reflowDir,
                reflowFragName,
                "java",
                Fragment.templateTypeJava(packageHelper.getPackageId(), "reflow"),
            )
            activityWriter.createFile(
                settingsDir,
                settingsFragName,
                "java",
                Fragment.templateTypeJava(packageHelper.getPackageId(), "settings"),
            )
            activityWriter.createFile(
                transformDir,
                transformFragName,
                "java",
                Fragment.templateTypeTransformJava(packageHelper.getPackageId()),
            )
            activityWriter.createFile(
                slideshowDir,
                slideshowFragName,
                "java",
                Fragment.templateTypeJava(packageHelper.getPackageId(), "slideshow"),
            )
            ...
            activityWriter.createFile(
                ...
                reflowModelName,
                "java",
                ViewModel.templateTypeJava(packageHelper.getPackageId(), "reflow"),
            )
            activityWriter.createFile(
                slideshowDir,
                slideshowModelName,
                "java",
                ViewModel.templateTypeJava(packageHelper.getPackageId(), "slideshow"),
            )
            activityWriter.createFile(
                transformDir,
                transformModelName,
                "java",
                ViewModel.templateTypeTransformJava(packageHelper.getPackageId()),
            )
            activityWriter.createFile(
                settingsDir,
                settingsModelName,
                "java",
                ViewModel.templateTypeJava(packageHelper.getPackageId(), "settings"),
            )
          }
          ...
          val projectInfo =
              projectManager.createTemplateProjectInfo(
                  ...
                  projectType = "Responsive activity Activity",
              )
          ...
          withContext(Dispatchers.Main) {
            listener?.onTemplateCreated(
                true,
                "Responsive activity Activity project created successfully at ${projectRoot.absolutePath}",
            )
  ...
  /** Copy resource files from assets/ResponsiveActivity/resources to app/src/main/res */
