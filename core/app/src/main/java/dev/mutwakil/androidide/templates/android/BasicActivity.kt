...
class BasicActivity : Template {
  override val displayName = "Basic Activity"
  ...
  // Asset paths
  private val ASSETS_BASE_PATH = BasicActivity::class.simpleName
  ...
  override suspend fun create(
      ...
      withContext(Dispatchers.IO) {
        try {
          Log.d("BasicActivity", "create() called - START")
          ...
          if (!structResult.success) {
            Log.e("BasicActivity", "Structure creation failed: ${structResult.message}")
            withContext(Dispatchers.Main) {
          ...
          val mainActivityContent =
              if (options.languageType == LanguageType.KOTLIN)
                  dev.mutwakil.androidide.templates.android.basic.BasicSources.mainActivityKotlin(
                      packageHelper.getPackageId()
                  )
              else
                  dev.mutwakil.androidide.templates.android.basic.BasicSources.mainActivityJava(
                      packageHelper.getPackageId()
                  )
          ...
          Log.d("BasicActivity", "Project created successfully")

          withContext(Dispatchers.Main) {
            listener?.onTemplateCreated(
                true,
                "Basic Activity project created successfully at ${projectRoot.absolutePath}",
            )
  ...
  private fun copyWrapperFiles(context: Context, projectRoot: File) {
    try {
      Log.d("BasicActivity", "Copying wrapper files...")
      ...
      Log.d("BasicActivity", "Wrapper files copied successfully")
    } catch (e: Exception) {
      Log.e("BasicActivity", "Error copying wrapper files", e)
      // Don't throw, just log - wrapper files are optional
    }
  }

  /** Copy resource files from assets/BasicActivity/resources to app/src/main/res */
  private fun copyResourceFiles(context: Context, projectRoot: File) {
    try {
      Log.d("BasicActivity", "Copying resource files...")

      val resDestDir = File(projectRoot, "app/src/main/res")
      copyAssetFolder(context, ASSETS_RESOURCES_PATH, resDestDir)

      Log.d("BasicActivity", "Resource files copied successfully")
    } catch (e: Exception) {
      Log.e("BasicActivity", "Error copying resource files", e)
  ...
  private fun copyAssetFolder(context: Context, assetPath: String, destDir: File) {
    try {
      ...
      Log.e("BasicActivity", "Error copying asset folder: $assetPath", e)
    }
  }
  ...
  private fun copyAssetFile(context: Context, assetPath: String, destFile: File) {
    try {
      ...
      Log.d("BasicActivity", "Copied: $assetPath -> ${destFile.absolutePath}")
    } catch (e: Exception) {
      Log.e("BasicActivity", "Error copying file: $assetPath", e)
      throw e
    }
  }
}
